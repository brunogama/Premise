...
func equalSeedsProduceEqualTraces() {
  var lhs = PremiseData(provider: PseudoRandomProvider(seed: 7, maxDraws: 32))
  var rhs = PremiseData(provider: PseudoRandomProvider(seed: 7, maxDraws: 32))
...
func replayProviderReproducesSequence() {
  var source = PremiseData(provider: PseudoRandomProvider(seed: 21, maxDraws: 32))
  let original = drawSample(using: &source)
  let trace = source.snapshot()

  var replay = PremiseData(provider: ReplayProvider(trace: trace))
...
private func drawSample(using data: inout PremiseData) -> DrawSample {
